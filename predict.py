...
def analyze_prob(df):
    result = {}
    for row in df[['length','infer_path','true_path','prob']].values:
        length = -row[0]
        ipath = row[1]
        tpath = row[2]
        prob = row[3]
        try:
            tmp = result[tpath]
            tmp.append([prob,length,ipath])
            result[tpath] = tmp
        except:
            result[tpath] = [[prob,length,ipath]]
    res = {}
...
def analyze_jaccard(df):
    ...
    for row in df[['length','infer_path','true_path','pred']].values:
        ...
        try:
            tmp = result[tpath]
            tmp.append([pred,ipath])
            result[tpath] = tmp
        except:
            result[tpath] = [[pred,ipath]]
    res = {}
    for key in result.keys():
        tmp = result[key]
    ...
    return analyze_dict,mean_jaccard_by_length
...
def feature_analyze(test_ratio,features,current_dir,mark):
    ...
    for name in name_list: 
        ...
        for file in file_list:
            ...
            result_by_length_file  = current_dir + '/result/test' + '/%s_dt_%s_%s.csv'%(mark,test_name,test_ratio)
